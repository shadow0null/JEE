...
from jee_engine import physics_v2
...
_LEADING_COMMAND_WORDS = re.compile(
    r"^\s*(?:please\s+)?(?:calculate|compute|solve|simplify|factor(?:ize)?|expand|differentiate|integrate|find)\s+",
    re.IGNORECASE,
)
...
_MATH_OPS = {
    ...
    "inequality": lambda p: math_engine.solve_inequality(p["expression"], p.get("var", "x")),
    "ap": lambda p: math_engine.arithmetic_progression(p["a"], p["d"], p["n"]),
    "gp": lambda p: math_engine.geometric_progression(p["a"], p["r"], p["n"]),
    "binomial_coefficient": lambda p: math_engine.binomial_coefficient(p["n"], p["r"]),
}
...
_PHYSICS_OPS = {
    ...
    "formula": lambda p: physics_v2.solve_formula(p["formula_id"], p.get("values", {}), p.get("unknown")),
}
